...
public class InvitedEntrantsActivity extends AppCompatActivity
    ...
    // startDate loaded from Firestore — used to enable/disable cancel button
    ...
    private void processDocument(DocumentSnapshot doc) {
        ...
        // ── Read startDate ────────────────────────────────────────────────────
        Timestamp startTs = doc.getTimestamp("eventStart");
        eventStartDate = (startTs != null) ? startTs.toDate() : null;

        // Pass startDate to adapter so it can decide enabled/disabled per row
        adapter.setEventStartDate(eventStartDate);
